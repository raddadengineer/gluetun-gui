...
- **JWT signing:** Set **`JWT_SECRET`** (and optionally **`JWT_EXPIRES_IN`**, e.g. `12h` or `7d`) on the **gluetun-gui** container environment. If unset, the server generates a persistent random secret saved to `${DATA_DIR}/jwt-secret` so sessions persist across restarts (or an ephemeral in-memory secret if `DATA_DIR` is unset).
